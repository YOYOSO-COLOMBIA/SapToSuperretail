...
module.exports = {
  sap: {
    ...
    businessPartnersPath: process.env.SAP_BP_PATH || '/b1s/v1/BusinessPartners',
    invoicesPath: process.env.SAP_INVOICES_PATH || '/b1s/v1/Invoices',
    creditNotesPath: process.env.SAP_CREDIT_NOTES_PATH || '/b1s/v1/CreditNotes',
    incomingPaymentsPath: process.env.SAP_INCOMING_PAYMENTS_PATH || '/b1s/v1/IncomingPayments',
    timeoutMs: Number(process.env.SAP_TIMEOUT_MS || 300000),
    rejectUnauthorized: bool(process.env.SAP_REJECT_UNAUTHORIZED, false),
    paginationConcurrency: Math.max(1, num(process.env.SAP_PAGINATION_CONCURRENCY, 1) || 1),
  ...
  app: {
    ...
    salesCreditNoteSeries: Number(process.env.SALES_CREDIT_NOTE_SERIES || 117),
